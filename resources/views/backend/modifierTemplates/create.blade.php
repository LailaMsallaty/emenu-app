<x-backend>
    ...
    <div class="col-md-12 mt-3">
        ...
        <form action="{{ Route('modifiertemplates.store') }}" method="post"  id="idForm">
            @csrf
            @method('post')
            <div class="row">
                <div class="card-body">
                    ...
                    <div class="col-6">
                        ...
                        <input required id="templateName" class="form-control" type="text" name="templateName" />
                    </div>
                    <br>
                    <div class="repeater form-group col-md-12">
                            <div data-repeater-list="List_modifiers">
                                <div data-repeater-item>
                                    <div class="row">
                                        @foreach (App::make('languages') as $key => $lang)
                                        <div class="col langforms" data-id="{{ $key }}">
                                            <label for="Name_{{ $key }}"
                                                class="">{{ __('Modifier Name') }} - {{ $key }}
                                                : <span class="text-danger">*</span></label>
                                            <input  class="form-control" type="text" name="modifier_name[{{$key}}]" />
                                        </div>
                                        @endforeach
                                        <div class="col">
                                            <label>{{  __('Modifier Price') }}
                                                : <span class="text-danger">*</span></label>
                            ...
                            <div class="mt-35">
                                <div class="add_record" >
                                    <input  class="btn btn-dark btn-sm " data-repeater-create type="button" value="{{ __('Add Modifier') }}"/>
                                </div>
                             </div>
                        </div>
                        <br>
